...
public:
	...
	UFUNCTION(
		BlueprintCallable,
		Category = "WaveVR|Gesture",
		meta = (ToolTip = "Get the hand confidence (0~1), 1 means the best accuracy."))
	static float GetHandConfidence(EWaveVRGestureHandType hand);

	UFUNCTION(
		BlueprintCallable,
		Category = "WaveVR|Gesture",
		meta = (ToolTip = "Get the hand pinch strength (0~1), 1 means the thumb and index finger tip are connected."))
		static float GetHandPinchStrength(EWaveVRGestureHandType hand);

	UFUNCTION(
		BlueprintCallable,
		Category = "WaveVR|Gesture",
		meta = (ToolTip = "Get the hand pinch origin location in the world space."))
		static FVector GetHandPinchOrigin(EWaveVRGestureHandType hand);

	UFUNCTION(
		BlueprintCallable,
		Category = "WaveVR|Gesture",
		meta = (ToolTip = "Get the hand pinch direction in the world space."))
		static FVector GetHandPinchDirection(EWaveVRGestureHandType hand);
};
