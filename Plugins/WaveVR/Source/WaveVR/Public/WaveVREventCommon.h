...
public:
	...
	UFUNCTION(
		BlueprintCallable,
		Category = "WaveVR|Event",
		meta = (ToolTip = "To notify the engine to reset button states on resume."))
	static void ShouldResetButtonStatesOnResume(bool reset);

private:
	static bool bIsLeftHanded;
	static bool bCheckLeftHanded;
	static bool bResetAllButtonStates;
#pragma endregion WVR_EventType_DeviceRoleChanged

#pragma region
public:
	static FInputRequestChangeNative OnInputRequestChangeNative;
	UFUNCTION()
	void OnInputRequestChangeHandling(uint8 device);
	//UPROPERTY(BlueprintAssignable, Category = "WaveVR")
	//FInputRequestChangeBp OnInputRequestChangeBp;
private:
	static bool bResetHmdButtonStates;
	static bool bResetRightButtonStates;
	static bool bResetLeftButtonStates;
#pragma endregion SetInputRequest
