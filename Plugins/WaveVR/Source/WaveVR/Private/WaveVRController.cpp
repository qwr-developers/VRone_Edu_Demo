...
void Device::Update_PressState(EWVR_InputId id)
{
	...
	{
		FWaveVRHMD* _hmd = FWaveVRHMD::GetInstance();
		if (_hmd != nullptr && _hmd->IsDeviceConnected((WVR_DeviceType)Hand))
		{
...
bool UWaveVRController::IsRightControllerConnected()
{
	...
	if (WaveVRDirectPreview::IsDirectPreview() || !GIsEditor) {
		FWaveVRHMD* _hmd = FWaveVRHMD::GetInstance();
		if (_hmd != nullptr && _hmd->IsDeviceConnected(WVR_DeviceType_Controller_Right))
			_connected = true;
	}
...
bool UWaveVRController::IsLeftControllerConnected()
{
	...
	if (WaveVRDirectPreview::IsDirectPreview() || !GIsEditor) {
		FWaveVRHMD* _hmd = FWaveVRHMD::GetInstance();
		if (_hmd != nullptr && _hmd->IsDeviceConnected(WVR_DeviceType_Controller_Left))
			_connected = true;
	}
...
bool UWaveVRController::IsControllerConnected(EWVR_DeviceType device)
{
	...
	if (WaveVRDirectPreview::IsDirectPreview() || !GIsEditor)
	{
		...
		{
		case EWVR_DeviceType::DeviceType_Controller_Right:
			_connected = _hmd->IsDeviceConnected(WVR_DeviceType_Controller_Right);
			break;
		case EWVR_DeviceType::DeviceType_Controller_Left:
			_connected = _hmd->IsDeviceConnected(WVR_DeviceType_Controller_Left);
			break;
		default:
			break;
		}
