...
enum class EFoveatedRenderingMode : uint8
{
	Disable = 0x0,
	Enable = 0x1,
	Default = 0x2,    /*Enable/Disable up to devices*/
};
...
public:
	...
	UPROPERTY(EditAnywhere, config, Category = Render, meta = (
		...
		ToolTip = "Foveated Rendering helps save device power. Please Ignore the following foveated parameters if you select Disable or Default. The foveated parameters depend on device if you select Default."))
	EFoveatedRenderingMode FoveatedRenderingMode;

	UPROPERTY(EditAnywhere, config, Category = Render, meta = (
		editcondition="bFoveatedParamsEditable",
	...
	UPROPERTY(EditAnywhere, config, Category = Performance, meta = (
		...
		ToolTip = "Automatically enable/disable foveation rendering and adjust foveation PeripheralQuality if needed."))
	bool bAdaptiveQualityAutoFoveation;

public:
	UPROPERTY()
	bool bFoveatedParamsEditable;
