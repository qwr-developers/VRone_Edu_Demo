...
float UWaveVRGestureBPLibrary::GetHandConfidence(EWaveVRGestureHandType hand)
{
	TSharedPtr< class FWaveVRGestureInputDevice > gestureInputDevice = GetGestureDevice();
	if (gestureInputDevice.IsValid() && gestureInputDevice->IsGestureInputDeviceInitialized())
		return gestureInputDevice->GetHandConfidence(hand);
	return 0;
}

float UWaveVRGestureBPLibrary::GetHandPinchStrength(EWaveVRGestureHandType hand)
{
	TSharedPtr< class FWaveVRGestureInputDevice > gestureInputDevice = GetGestureDevice();
	if (gestureInputDevice.IsValid() && gestureInputDevice->IsGestureInputDeviceInitialized())
		return gestureInputDevice->GetHandPinchStrength(hand);
	return 0;
}

FVector UWaveVRGestureBPLibrary::GetHandPinchOrigin(EWaveVRGestureHandType hand)
{
	TSharedPtr< class FWaveVRGestureInputDevice > gestureInputDevice = GetGestureDevice();
	if (gestureInputDevice.IsValid() && gestureInputDevice->IsGestureInputDeviceInitialized())
		return gestureInputDevice->GetHandPinchOrigin(hand);
	return FVector::ZeroVector;
}

FVector UWaveVRGestureBPLibrary::GetHandPinchDirection(EWaveVRGestureHandType hand)
{
	TSharedPtr< class FWaveVRGestureInputDevice > gestureInputDevice = GetGestureDevice();
	if (gestureInputDevice.IsValid() && gestureInputDevice->IsGestureInputDeviceInitialized())
		return gestureInputDevice->GetHandPinchDirection(hand);
	return FVector::ZeroVector;
}
